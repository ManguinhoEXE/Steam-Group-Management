...
async def get_all_users(
    ...
    users_data = []
    for user in users:
        total_deposits = db.query(func.sum(Deposit.amount)).filter(Deposit.member_id == user.id).scalar() or 0
        
        total_expenses = db.query(func.sum(PurchaseShare.share_amount)).filter(PurchaseShare.member_id == user.id).scalar() or 0
